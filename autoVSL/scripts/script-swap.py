...
# minimax models need a paid voice clone ($1.50) first; f5 zero-shot clones from
# a reference clip of the source audio for free ($0.05/1k chars total).
TTS_ENDPOINTS = {
...
LIPSYNC_ENDPOINTS = {
    "pro": "fal-ai/sync-lipsync/v2/pro",        # premium, most expensive
...
def stage_speak(work: Path, voice_id: str | None, tts: str = "hd",
    ...
    if tts == "f5":
        ...
        if video is None or not video.is_file():
            raise SystemExit("F5-TTS needs the source video for the voice reference")
        wav = extract_audio(video, work / "source-audio.wav")
        ...
        if not url:
            raise RuntimeError(f"No audio url in F5 response: {result}")
        tmp = work / "new-vo.f5.wav"
        download(url, tmp)
...
def main() -> int:
    ...
    if args.stage in ("run", "transcribe"):
        ...
        print(f"\n--- transcript ---\n{text}\n------------------")
    if args.stage in ("run", "clone") and not (args.stage == "run" and args.tts == "f5"):
        voice_id = stage_clone(video, work)   # f5 clones zero-shot in speak — no paid clone
