#!/usr/bin/env python3
"""Poster Studio — vision planner (MODE B / NEW_GENERATION only).

Reads a product image + text brief, runs a vision LLM (headless claude CLI
with the Read tool, or fal.ai's openrouter/router/vision), and emits the
structured text the compositor + Nano Banana need:
...
def run_gemini(model: str, system_prompt: str, user_prompt: str,
               product_image: str, object_image: str | None) -> str:
...
def main() -> int:
    ...
    catalog = _load_layout_templates(args.layout_json)
